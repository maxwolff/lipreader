...
def train(train_dir, max_steps):
	...
	batch_size, videos, labels, assignOp, numPhonemes = cnn2layer_build.inputs(train_dir)
	print('input')
	logits = cnn2layer_build.inference(videos, batch_size, numPhonemes)
	print('logits')
	loss, classifications = cnn2layer_build.loss(logits, labels, batch_size)
	print('loss')
	train_op = cnn2layer_build.train(loss, global_step, batch_size)
	print('train')
