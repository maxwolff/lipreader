...
def interpolate(prev_frame, next_frame):
	#could not find an appropriate interpolating function
	return prev_frame
...
for speaker_index, speaker in enumerate(speakers):
	...
	for video_index, video in enumerate(videos):
		...
		#get frames for video and add them to frames
		...
		#retrieve phonemes for this speaker and video
		...
		for phoneme_index, phoneme in enumerate(phonemes):
			...
			#standardized phoneme frames to be the first, the middle and end frame from the phoneme video clip
			timed_frames = [phoneme_frames[0], phoneme_frames[len(phoneme_frames)/2], phoneme_frames[-1]]

			#variables used to recycle previous face location when a face was not detected on this frame
